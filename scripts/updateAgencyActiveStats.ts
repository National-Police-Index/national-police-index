...
async function updateAgencyActiveStats(startState?: string) {
  try {
    ...
    const lastState = progressDoc.exists()
      ? progressDoc.data()?.lastState || startState || "alabama"
      : startState || "alabama";

    const states = US_STATES.filter(
      (s: State) => s.reference === "kansas",
    ).filter((s: State) => s.hasData);
      ...
      try {
        ...
        for (const statDoc of statsSnapshot.docs) {
          ...
          try {
            ...
            const existingActiveStat = stats.find(
              (stat: Stat) => stat.label === "Total Active Officers",
            );
            ...
            if (batchCount >= 500) {
              batchCount = 0;
              console.log(
                `Processed ${processedCount} agencies in ${state.name}`,
              );
            }
          } catch (error) {
            console.error(
              `Error processing agency ${agencyId} in ${state.name}:`,
              error,
            );
          }
        }
        ...
        console.log(
          `Completed processing ${processedCount} agencies in ${state.name}`,
        );
      } catch (error) {
