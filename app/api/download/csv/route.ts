...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (textQuery) {
      ...
      q = query(q, where('searchable_text', 'array-contains', textQuery.toLowerCase()));
    }
    ...
    if (sortBy === 'name') {
      ...
      q = query(q, orderBy('start_date', sortOrder === 'desc' ? 'desc' : 'asc'));
    } else if (sortBy === 'agency') {
      q = query(q, orderBy('agency_name', sortOrder === 'desc' ? 'desc' : 'asc'));
    }
